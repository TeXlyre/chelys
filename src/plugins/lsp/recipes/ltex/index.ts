...
const BASE = `https://github.com/ltex-plus/ltex-ls-plus/releases/download/${VERSION}`;
...
const archiveFor = (os: string): string =>
	os === 'windows' ? `${DIR}-windows-x64.zip` : `${DIR}-${os}-x64.tar.gz`;
...
const extractStep = (os: string) =>
	os === 'windows'
		? {
				...
				command: 'tar',
				args: ['-xf', archiveFor(os)],
			}
		: {
				label: 'Extract archive',
				command: 'tar',
				args: ['-xzf', archiveFor(os)],
			};
			...
			buildSteps: [
				{
					...
					args: [
						...
						'-o',
						archiveFor('linux'),
						`${BASE}/${archiveFor('linux')}`,
					],
				},
				{
					label: 'Extract archive',
					command: 'tar',
					args: ['-xzf', archiveFor('linux')],
				},
				{
				...
				{
					label: 'Stage proxy binary',
					command: 'cp',
					args: ['bin/lsp-ws-proxy', 'lsp-ws-proxy'],
				},
				{

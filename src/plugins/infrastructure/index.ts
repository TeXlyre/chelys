...
const unixInstall = (asset: string) => [
    {
        ...
        command: 'curl',
        args: ['-fL', '-o', '${CHELYS_DATA}/traefik.tar.gz', `${TRAEFIK_BASE}/${asset}`],
    },
    {
        label: 'Extract Traefik',
        command: 'tar',
        args: ['-xzf', '${CHELYS_DATA}/traefik.tar.gz', '-C', '${CHELYS_DATA}', 'traefik'],
    },
];
...
const traefikRecipe: Recipe = {
    ...
    modes: [
        {
            kind: 'system',
            installSteps: unixInstall(`traefik_${TRAEFIK_VERSION}_linux_amd64.tar.gz`),
            runCommand: {
            ...
            platforms: {
                macos: {
                    installSteps: unixInstall(`traefik_${TRAEFIK_VERSION}_darwin_arm64.tar.gz`),
                },
                windows: {
                    installSteps: [
                        {
                        ...
                        },
                        {
                            label: 'Extract Traefik',
                            command: 'tar',
                            args: ['-xf', '${CHELYS_DATA}/traefik.zip', '-C', '${CHELYS_DATA}', 'traefik.exe'],
                        },
                    ],
                    runCommand: {
                    ...
                    uninstallSteps: [
                        {
                            label: 'Remove Traefik',
                            command: 'cmd',
                            args: ['/c', 'del', '/q', '${CHELYS_DATA}\\traefik.exe', '${CHELYS_DATA}\\traefik.zip'],
                        },
                    ],
                },
            },

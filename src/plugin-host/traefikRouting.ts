...
export function rewriteTransportUrl(
	config: TransportConfig,
): string | undefined {
	const url =
		typeof config.transportUrl === 'string' ? config.transportUrl : undefined;
	if (!url) return url;
	...
	if (getStoredSetting<boolean>('traefikEnabled')) {
		...
		const scheme = toTransportScheme(
			base.scheme,
			toTransportScheme(original.scheme, 'ws'),
		);

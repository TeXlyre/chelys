...
export class PeerSession {
	...
	private async attachCollab(): Promise<void> {
		...
		doc
			.getArray<FileSyncRequest>('syncRequests')
			.observe(() => this.onRequestsChange());
		doc
			.getArray<FileSyncVerification>('verifications')
			.observe(() => this.onVerificationsChange());
	...
	private emit(patch: Partial<PeerStats>): void {
		...
		for (const listener of this.statsListeners) listener(snapshot);
	}
	...
	private purgeDisconnectedPeers(): void {
		...
		const holdSignalsArray =
			this.doc.getArray<FileSyncHoldSignal>('holdSignals');
		for (let i = holdSignalsArray.length - 1; i >= 0; i--) {
			const signal = holdSignalsArray.get(i);
			if (
				gone.has(signal.targetPeerId) ||
				(signal.holderId && gone.has(signal.holderId))
			) {
	...
	private async handleIncomingSyncRequest(
	...
	private async handleSyncRequestUpdate(
		request: FileSyncRequest,
	): Promise<void> {
	...
	private issueHoldSignal(targetPeerId: string): FileSyncHoldSignal | null {
		...
		const timer = setTimeout(
			() => this.clearHold(targetPeerId),
			cfg.holdTimeoutSeconds * 1000,
		);
	...
	private isRequestExpired(request: FileSyncRequest): boolean {
		return (
			...
			getPeerConfig().requestTimeoutSeconds * 1000
		);
	}

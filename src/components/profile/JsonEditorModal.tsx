...
type StoreShape = 'versioned' | 'array' | 'plain';

const SHAPES: Record<EditableType, StoreShape> = {
	settings: 'versioned',
...
const JsonEditorModal: React.FC<JsonEditorModalProps> = ({
	...
	useEffect(() => {
		...
		const load = () => {
			const data = getUserData<unknown>(userId, type);
			if (SHAPES[type] === 'versioned') {
			...
			setText(
				JSON.stringify(data ?? (SHAPES[type] === 'array' ? [] : {}), null, 2),
			);
		};
	...
	const handleSave = () => {
		...
		try {
			...
			setParseError(error instanceof Error ? error.message : t('Invalid JSON'));
			return;
		}
		...
		if (!valid) {
			setParseError(
				shape === 'array'
					? t('Top-level value must be a JSON array')
					: t('Top-level value must be a JSON object'),
			);
			return;
		}
		try {
